...
KeyFrame::KeyFrame(int timestamp, Frame* frame) :
    ...
    this->frame = frame;
    M1 = Mat(
            Matx34d(7.188560000000e+02, 0.000000000000e+00, 6.071928000000e+02, 0.000000000000e+00,
...
Mat KeyFrame::reconstructFromPrevFrame() {
    ...
    cout << "xxxxxxxxxxxxxxx inside reconstruct from prev frame" << endl;
    ...
    //imshow("prev_kf", prev_kf->getFrame()->getFrame());
    //waitKey(0);
    cout << "Size of point3D = " << point3D.size() << endl;
    return point3D;
}
...
Mat KeyFrame::getNew3DPoints() {
    ...
    uint32_t common_counter = 0;
    uint32_t prev_counter = 0;
    uint32_t bad_counter = 0;

    Mat ret(size_kp_curr, 5, CV_32FC1, Scalar(0));

    auto matches_curr = curr_kf_frame->getMatches();
    auto matches_prev = prev_kf_frame->getMatches();

    int idx = 0;
    uint32_t *match_type = new uint32_t[frame->kpts.size()];

    curr_kf_frame->matchFeatures(prev_kf_frame, &keyframe_match);
    cout << "keyframe_match.size() = " << keyframe_match.size() << endl;
    
    for (auto it : keyframe_match) {
        int id_curr = it.queryIdx;
        int id_prev = it.trainIdx;

        ret.at<float>(id_curr, 0) = 2;
        ret.at<float>(id_curr, 1) = pts_prev.at<float>(id_prev, 0);
        ret.at<float>(id_curr, 2) = pts_prev.at<float>(id_prev, 1);
        ret.at<float>(id_curr, 3) = pts_prev.at<float>(id_prev, 2);
        ret.at<float>(id_curr, 4) = id_prev;

        match_type[id_curr] = 2;
        prev_counter++;

        idx++;
    }

    // new_counter should be equal to prev_counter
    //for(auto it : curr_match) {
    int i = 0;

    for(auto it : getFrame()->kpts) {
        //int id_prev = it.trainIdx;
        //int id_curr = it.queryIdx; 
        int match_pos = 0;
        ...
        switch(match_type[i]) {
            ...
            default:
                ...
                match_pos = 0;
                for(auto ma : curr_match) {
                    if(ma.queryIdx == i) {
                        //match_type[i] = 1;
                        ret.at<float>(i, 0) = 1;
                        ret.at<float>(i, 1) = pts_curr.at<float>(match_pos, 0);
                        ret.at<float>(i, 2) = pts_curr.at<float>(match_pos, 1);
                        ret.at<float>(i, 3) = pts_curr.at<float>(match_pos, 2);
    ...
    // Summary
    // type 1 = new points
    // type 2 = common points

    cout << "size of keypoints = " << getFrame()->kpts.size() << endl;
    cout << "new_counter = " << new_counter << endl;
    cout << "common_counter = " << common_counter << endl;
    cout << "prev_counter = " << prev_counter << endl;
    cout << "bad_counter = " << bad_counter << endl;

    /*
    i=0;
    uint32_t moved_on = 0;
    for(auto it : matches_curr) {
        if(has3d_prev[i] >= 0) {
            // We're already dealt with this earlier
            moved_on++;
            continue;
        }
        ...
        ret.at<float>(id_curr, 1) = pts_curr.at<float>(idx, 0);
    ...
    cout << "idx = " << idx << endl;
    cout << "current_matches.size = " << curr_match.size() << endl;
    cout << "keyframe_match.size = " << keyframe_match.size() << endl;
    cout << "new counter = " << new_counter << endl;
    cout << "common counter = " << common_counter << endl;
    cout << "prev counter = " << prev_counter << endl;
    cout << "bad counter = " << bad_counter << endl;
    ...
    cout << "Done with the construction here" << endl;
